/* ═══════════════════════════════════════════════════════════════════
   Hotel Elizabeth — Coñaripe · demo 106 (tanda P3)

   JS clásico, IIFE, sin librerías. Hace tres cosas y nada más:
   1. Cancela el temporizador de rescate de la clase .js.
   2. Cabecera fantasma (el estado por defecto es el sólido; acá se
      agrega la transparencia sobre la portada y el esconder al bajar).
...
(function () {
  ...
  var portada  = document.querySelector('.lateral');
  var reduce   = window.matchMedia('(prefers-reduced-motion: reduce)');

  /* ═══════════════════════════════════════════════════════════════
     2 · CABECERA FANTASMA
     Transparente mientras se ve la portada; al bajar se esconde; al
     subir vuelve sólida. Umbral de 120 px y deltas menores a 8 px
     ignorados, para que no tirite con el scroll fino del trackpad.
     ═══════════════════════════════════════════════════════════════ */
  if (cab) {
    ...
    var pintar = function () {
      ...
      /* Transparencia: sólo mientras la portada ocupa la pantalla */
  ...
  var piezas = [].slice.call(document.querySelectorAll(
    '.bloque > *, .caso, .lateral__cuerpo > *, .dueno__cols > div'
  ));
