/* ═══════════════════════════════════════════════════════════════════
   Camping Cordillera — Coñaripe · demo 94 (tanda P3)
...
(function () {
  ...
  if (form && resumen && enviar) {
    var armar = function () {
      var que   = form.elements.grupo.value;
      var fecha = form.elements.fecha.value.trim();
      var gente = form.elements.cuantos.value.trim();

      var texto = 'Hola, quiero reservar en el camping. Venimos como ' + que.toLowerCase();
      if (fecha) { texto += ', ' + fecha; }
      if (gente) { texto += ' (' + gente + ')'; }
      texto += '. ¿Tienen disponibilidad?';

      resumen.textContent = texto;
      enviar.href = 'https://wa.me/56998219388?text=' + encodeURIComponent(texto);
    };
  ...
  var piezas = [].slice.call(document.querySelectorAll(
    '.lam__cuerpo > *, .grupo, .dueno__cols > div'
  ));
