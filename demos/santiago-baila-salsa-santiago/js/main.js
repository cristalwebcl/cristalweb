/* ══════════════════════════════════════════════════════════════════
   ...
   Sin JavaScript la página se lee entera: los cinco momentos son una
   lista, el paso básico está dibujado, las huellas de la portada
   laten en CSS puro y el botón de WhatsApp abre con un mensaje base.
   El JS agrega las apariciones al scroll, la cifra que cuenta, el
   mensaje de reserva armado con lo que se marca y el video de fondo.
...
(function () {
  ...
  var piezas = [];
  ['.cifras__lista', '.primera > .ancho:first-child', '.momentos', '.paso', '.llevar', '.llevar__lista', '.reserva',
   '.niveles .ancho > :not(.niveles__cols)', '.tabla-niveles', '.horario', '.tajo__txt',
   '.escuela .ancho > :not(.fichas)', '.fichas', '.cita__txt', '.mos .ancho', '.mos__grilla',
   '.reserva-sec__cols > *', '.dueno .ancho']
  ...
  /* ── 3 · Reserva: el mensaje ── */
  var nombre = document.getElementById('r-nombre');
  var ritmo = document.getElementById('r-ritmo');
  var nivel = document.getElementById('r-nivel');
  var cuando = document.getElementById('r-cuando');
  var salida = document.getElementById('r-salida');
  var btn = document.getElementById('r-wsp');
  if (nombre && ritmo && nivel && cuando && salida && btn) {
    var base = 'https://wa.me/56958587797';
    var armar = function () {
      var n = nombre.value.trim();
      var t = 'Hola' + (n ? ', soy ' + n : '') + '. Quiero ir a mi primera clase.';
      t += '\nRitmo: ' + ritmo.value;
      t += '\nNivel: ' + nivel.value;
      if (cuando.value.trim()) { t += '\nMe acomoda: ' + cuando.value.trim(); }
      t += '\n¿Cuándo puedo partir?';
      salida.textContent = (n || cuando.value.trim()) ? t : '';
      btn.setAttribute('href', base + '?text=' + encodeURIComponent(t));
    };
    [nombre, ritmo, nivel, cuando].forEach(function (c) {
  ...
  var portadaW = document.querySelector('.portada');
  if (wapp && portadaW) {
    var mirarWapp = function () {
      var r = portadaW.getBoundingClientRect();
    ...
    if ('IntersectionObserver' in window) {
      ...
      }, { threshold: [0, 0.55, 1] }).observe(portadaW);
    } else {
