/* ═══════════════════════════════════════════════════════════════════
   Ferretería Harcha · demo 120 (tanda P3)
...
(function () {
  ...
  var elLista   = document.getElementById('f-lista');
  ...
  if (elLocal && elLista && elResumen) {
    var armar = function () {
      var lista = elLista.value.replace(/\s+$/, '');
      var texto = 'Hola, quiero hacer un pedido.\n' +
                  'Dónde: ' + elLocal.value + '\n' +
                  'Lista: ' + (lista || '(sin escribir todavía)');

      elResumen.textContent = texto;

      /* Si algún día hay número, basta cambiar el href del HTML por el
         wa.me correspondiente y este mismo código le cuelga el texto. */
      if (elEnviar && DESTINO.indexOf('wa.me') !== -1) {
        elEnviar.href = DESTINO.split('?')[0] + '?text=' + encodeURIComponent(texto);
      }
    };

    elLocal.addEventListener('change', armar);
    elLista.addEventListener('input', armar);
    armar();
  }
  ...
  var piezas = [].slice.call(document.querySelectorAll(
    '.portada__texto > *, .locales__intro > *, .cols > .col, ' +
    '.despacho > *, .vuelta__cuerpo > *, .dueno__cols > div'
  ));
