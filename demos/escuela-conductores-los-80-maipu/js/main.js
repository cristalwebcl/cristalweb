/* Escuela de Conductores Los 80 — main.js
   IIFE, JS clásico, sin librerías, sin type="module" (rompe en file://).
   Hace tres cosas: cancelar el rescate, la cabecera fantasma, las
   apariciones al scrollear y encender el semáforo de la portada.
   Nada del contenido depende de este archivo: los tres caminos, la
   tabla de clases y los datos están escritos en el HTML. Si el script
   no llega, el semáforo se ve en verde y la página se lee entera. */
(function () {
  'use strict';
  ...
  var grupos = document.querySelectorAll(
    '.portada__cif, .caminos, .pasos, .tabla, .mitades__texto, ' +
    '.mos__grilla, .contacto__grid, .dueno__cols, .datos'
  );
  ...
  /* ── Escena: el semáforo se enciende en secuencia ────────────────
     Se observa la .escena entera y no los lentes: un lente encendido
     arranca con opacity 0, y aunque ahí sí tiene área, observar seis
     círculos para una sola secuencia no tiene sentido. Pasa una vez y
     queda en verde: un semáforo de portada que titila es ruido. Con
     menos movimiento, verde y listo. ── */
