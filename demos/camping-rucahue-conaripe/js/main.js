/* ═══════════════════════════════════════════════════════════════════
   Camping Rucahue — Coñaripe · demo 98 (tanda P3)
...
(function () {
  ...
  if (form && resumen && enviar) {
    var armar = function () {
      var que   = form.elements.quincho.value;
      var fecha = form.elements.fecha.value.trim();
      var gente = form.elements.cuantos.value.trim();

      var texto = 'Hola, quiero reservar en el camping';
      if (fecha) { texto += ' ' + fecha; }
      if (gente) { texto += '. Somos ' + gente; }
      texto += '. Quincho: ' + que.toLowerCase() + '. ¿Tienen disponibilidad?';

      resumen.textContent = texto;
      enviar.href = 'https://wa.me/56985803840?text=' + encodeURIComponent(texto);
    };
  ...
  var piezas = [].slice.call(document.querySelectorAll(
    '.lam__cuerpo > *, .plan__item, .dueno__cols > div'
  ));
